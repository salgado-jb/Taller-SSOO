...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	unsigned int inode_table_dir = block_group(group_number)->inode_table;
	int inodes_per_block = block_size / _superblock->inode_size;

	int target_block = inode_table_dir + (inode_index_in_group / inodes_per_block);
